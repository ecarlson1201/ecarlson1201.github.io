...
const generateQuestion = function () {
    ...
    QUESTION.text(questionsList[counter.indexOf()].question);

    INPUT.each(function (index) {
        $(this).val(questionsList[counter.indexOf()].options[index]);
    });

    LABEL.each(function (index) {
        $(this).text(questionsList[counter.indexOf()].options[index]);
    });
};
...
const getVidLink = function () {
    if (!questionsList[counter.indexOf()].link === false) {
        VIDEO.removeClass('hidden');
